...
def initialize_readme(target: Path, competition_name: str, template_name: str) -> None:
    ...
    readme.write_text(
        ...
        "## Notes\n\n"
        "- Add competition rules and data notes here.\n"
        "- Keep the implementation in `src/`.\n"
        "- Track experiments in `experiment_log.md`.\n",
        encoding="utf-8",
    )


def initialize_experiment_log(target: Path) -> None:
    log = target / "experiment_log.md"
    log.write_text(
        "## exp001\n\n"
        "Date:\n"
        "Competition:\n\n"
...
def create_competition(name: str, template_name: str) -> Path:
    ...
    initialize_experiment_log(target)
    return target
